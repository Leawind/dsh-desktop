...
设置界面使用与 DSH 设置页一致的居中双栏面板。左侧是分区标签导航，右侧由固定关闭入口和可滚动设置项组成；设置项采用左侧标题与说明、右侧控件的行布局。初始标签包括“当前窗口”“全局设置”和“DSH 服务”，后续全局管理功能继续按职责增加分区。

设置数据仍分为“当前窗口”和“全局设置”两个作用域。“DSH 服务”标签展示 Desktop Host 维护的全局服务状态。
