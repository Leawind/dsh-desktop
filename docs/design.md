...
设置界面使用与 DSH 设置页一致的居中双栏面板。左侧是分区标签导航，右侧由固定关闭入口和可滚动设置项组成；设置项采用左侧标题与说明、右侧控件的行布局。标签包括“当前窗口”“已打开窗口”“全局设置”“DSH 服务”和“DSH 运行时”。

设置界面分别呈现当前窗口状态、Desktop Host 的全局运行状态和持久化的全局设置。“已打开窗口”“DSH 服务”和“DSH 运行时”标签均展示 Host 级信息。
...
- 查看和修改当前窗口的 DSH URL；
- 从 Endpoint Registry 中选择最近连接的服务作为当前窗口的新目标。
