...
- 手动检查与当前发行变体、平台和架构匹配的 DSH Desktop 更新，并在确认后安装和重启。

应用更新从 GitHub Release 的最新稳定版本读取发布信息，并校验下载资产的 SHA-256 digest。更新文件先保存到应用数据目录，再由临时更新助手在 Host 退出后替换当前可执行文件并启动新版。更新会关闭应用窗口并停止全部受管 DSH 服务。当前可执行文件所在目录不可写时，应用报告无法自动安装，用户可从 Release 页面手动下载替换。
