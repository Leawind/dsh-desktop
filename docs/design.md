...
DSH Desktop 同一用户会话只运行一个 Host。首次启动创建 Host、托盘图标和第一个浏览器窗口；之后启动可执行文件会通过仅绑定回环地址、带随机令牌的本地 IPC 请求 Host 创建新窗口，随后立即退出。Host 管理全部浏览器窗口、托盘、DSH 服务端点和 DSH 运行时。关闭最后一个浏览器窗口后 Host 保持运行，直到用户从托盘选择“退出”。
...
窗口关闭只会解除该窗口与服务的连接。Host 托盘菜单提供“打开新窗口”和“退出”。“打开新窗口”始终创建一个窗口；“退出”先关闭全部窗口，再退出 Host 并停止所有由应用管理的 DSH 服务。
