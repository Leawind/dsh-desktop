...
DSH Desktop 同一用户会话只运行一个 Host。Host 持有操作系统级排他锁，并通过仅绑定回环地址、带随机令牌的本地 IPC 接收后续启动请求以创建新窗口；排他锁在 Host 退出时由操作系统释放。首次启动创建 Host、托盘图标和第一个浏览器窗口。Host 管理全部浏览器窗口、托盘、DSH 服务端点和 DSH 运行时。每个浏览器窗口在首次连接后每 250 ms 通过本地 bridge 报告活动；连续 1 秒未报告的窗口会被回收。关闭最后一个浏览器窗口后，Host 在仍有受管 DSH 服务时保持运行；全部受管服务被空闲回收后 Host 自动退出。
