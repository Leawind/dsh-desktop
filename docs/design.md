...
两种变体均作为单个可执行文件发行。`bundled` 是面向多数用户的推荐下载，`slim` 提供更小的可执行文件。发行变体是构建产物的固有能力，不是可以在运行时修改的全局设置。

两个变体使用相同的应用版本号，通过发行包文件名中的 `bundled` 或 `slim` 字段区分。

### 应用数据位置

开发构建始终使用平台标准目录。安装包在可执行文件同级部署非空的 `dsh-desktop.installed` 标记文件，安装后的应用同样使用平台标准目录。

没有该标记的直接运行文件为严格便携版。Host 以 `current_exe()` 的父目录为基准，将自身的全部持久化状态写入同级 `data/`：

```text
<可执行文件目录>/
├── dsh-desktop-…-portable
└── data/
    ├── settings.json
    ├── instance.lock
    ├── embedded-resources/
...
便携版首次启动使用默认设置；`data/` 无法创建或写入时启动失败，不会改用系统目录。设置中保存的 DSH Home 选择也随 `data/settings.json` 携带，但 DSH Home 的实际内容、浏览器数据及用户选择的其他外部路径仍由其原位置管理。

## 总体架构

DSH Desktop 同一用户会话只运行一个 Host。Host 持有操作系统级排他锁，并通过仅绑定回环地址、带随机令牌的本地 IPC 接收后续启动请求以创建新窗口；排他锁在 Host 退出时由操作系统释放。首次启动创建 Host、托盘图标和第一个浏览器窗口。Host 管理全部浏览器窗口、托盘、DSH 服务端点和 DSH 运行时。每个浏览器窗口在首次连接后每 250 ms 向本地 bridge 的心跳端点报告活动；连续 1 秒未报告的窗口会被回收。业务命令通过固定大小且有界的标准线程工作队列执行，避免请求无限制地创建线程。后台监控负责服务健康状态和系统外观探测，快照请求只读取当前状态。托盘退出、空闲回收和终止信号都通过同一有序关闭流程回收窗口、bridge 和受管进程。关闭最后一个浏览器窗口后，Host 在仍有受管 DSH 服务时保持运行；全部受管服务被空闲回收后 Host 自动退出。
...
窗口关闭只会解除该窗口与服务的连接。Host 托盘菜单提供“打开新窗口”和“退出”。“打开新窗口”始终创建一个窗口；“退出”先关闭全部窗口，再退出 Host 并停止所有由应用管理的 DSH 服务。没有窗口且全部受管服务被空闲回收时，Host 自动退出。
...
每个主窗口由唯一 Host 通过 WebUI 在已安装浏览器中创建，加载仅绑定回环地址的 DSH Desktop 前端。开发模式由 Vite 提供页面与热更新，并将 `/api` 代理到 Host 的固定回环 bridge 端口；发行模式由 Host 提供嵌入资源。首次加载使用带随机令牌的窗口会话 URL；前端为本地 bridge HTTP 命令设置 `X-DSH-Desktop-Token` 请求头，并在窗口控制 WebSocket 建连时携带同一令牌。bridge 同时验证令牌和窗口标识，浏览器不获得文件或进程权限。DSH Web UI 使用填满内容区的 iframe 加载。
...
设置界面使用与 DSH 设置页一致的居中双栏面板。左侧是分区标签导航，右侧由固定关闭入口和可滚动设置项组成；设置项采用左侧标题与说明、右侧控件的行布局。设置页包含“界面”“DSH”“启动”“运行时”和“关于”五项。

设置界面分别呈现持久化的全局设置、Desktop Host 的全局运行状态和应用元数据。

设置项修改后自动校验并应用。选择类设置立即保存；文本和数字输入在控件失焦或设置层关闭时保存。Desktop Host 持久化全局设置，并将结果同步到所有已打开窗口。

#### 当前窗口

当前窗口的 DSH URL 由主窗口状态栏编辑。状态栏始终显示当前 URL，使多窗口同时打开时仍能明确识别操作对象。
...
重启或停止共享进程时，界面显示目标 URL 和受影响的窗口数量。
...
设置功能位于 `features/settings`。`SettingsOverlay` 只负责设置层外壳、标签导航、页面装配与关闭时提交；“界面”“DSH”“启动”“运行时”“关于”分别由独立页面组件实现。全局设置草稿及自动保存、当前窗口 URL 校验及自动连接各自封装为 composable，页面组件仅处理本页的控件和显示。

页面组件不承担进程控制、持久化或 URL 规范化逻辑。通用交互元素封装为可复用 Vue 组件，包括按钮、输入框、切换项、对话框、状态标记和错误提示。选择器和数字输入直接使用 Element Plus 原生控件，并通过项目主题统一样式。功能页面不重复实现基础交互和样式。
...
组件库维护 100ms、200ms、300ms 三档运动时长和统一缓动曲线，并遵循 DSH 对各组件的实际使用方式；DSH 中没有动画的交互状态不额外添加过渡。按钮、输入框和状态标记等已有直接对应实现的组件优先逐项对齐。选择器使用 Element Plus 原生控件，并通过项目的 Element Plus 主题对齐输入框的尺寸与状态规则以及菜单的表面、阴影、间距和交互状态。
...
- 只有 DSH Desktop 的本地顶层页面从窗口会话 URL 取得 bridge 令牌；
- iframe 中的 DSH 页面不获得 bridge 会话或 Host bridge 对象；
- 应用不向 iframe 注入 Host 命令、进程控制函数或本地文件访问能力；
- iframe 的 `src` 只由应用在校验窗口 URL 后设置；
- 修改窗口 URL 时先校验协议和地址，再更新 iframe 和加载策略。

开发构建可以使用浏览器开发者工具选中和调试 DSH iframe。

DSH 响应需要允许浏览器 iframe 嵌入，才能在主窗口展示。
...
运行时构建由确切的 Node.js、DSH 和 pnpm 版本及生产依赖 lockfile 定义。构建过程校验 Node.js 官方摘要、npm 包 integrity、允许执行的依赖安装脚本、三个入口的实际版本和关键文件摘要。Host 启动时只物化前端、图标和运行时清单，以便尽快显示窗口；首次实际使用内置 DSH 时才物化种子归档，将其安装到版本隔离目录并再次校验。运行时定义未变化时，开发和发行构建复用已经准备好的种子目录。

`slim` 发行变体不包含或管理内置运行时。它可以使用 `none`、`system`、`custom` 和 `npx` 来源，并提供与 `bundled` 相同的窗口、连接和服务状态界面。

### 系统、自定义和 npx 运行时

`system` 在每次需要解析启动命令时从有效 `PATH` 查找 `dsh`。Unix 平台优先取得用户登录 Shell 的 PATH，失败时使用 Desktop Host 继承的 PATH；Windows 遵循进程环境和可执行文件扩展规则。Host 记录本次实际解析到的路径和版本，用于运行状态与诊断，不修改该安装。
...
更新失败时，Host 会恢复先前 Runtime 并重新启动已停止的受管服务。

## 版本兼容性

DSH 处于快速迭代阶段，命令行、Web 页面、运行时依赖和嵌入策略都可能发生破坏性变化。DSH Desktop 使用远程兼容性清单为当前应用版本指定一个经过验证的 DSH 更新目标，并以实际验证结果为准。

- 每个 `bundled` 应用版本在清单中拥有一个确切的 DSH 更新目标；
- Runtime 安装始终使用该确切版本；
- 更新后的 Runtime 会在隔离目录中验证 `dsh web --help` 和启动流程；
- `system`、`custom` 与 `npx` 来源不由内置 Runtime 更新机制修改；
- `none` 和外部服务无法取得可靠版本时，通过连接与 iframe 行为验证其可用性，不推断版本。

DSH Desktop 使用的上游行为、破坏影响和验证方式集中记录在 [DSH 兼容性契约](./dsh-compatibility.md) 中。
...
| 字段         | 含义                                  |
| ------------ | ------------------------------------- |
| `dshVersion` | 当前应用版本已验证的确切 DSH 更新目标 |
...
应用通过 HTTPS 获取仓库中的原始清单，并缓存最近一次成功获取的内容。远程清单中缺少当前应用版本时，应用继续尝试缓存和内置清单；所有清单均缺少该版本时不提供内置 Runtime 更新。

## 兼容性验证

一个 DSH 版本作为 `dshVersion` 更新目标前，需要在 Windows、Linux 和 macOS 上完成以下验证：
...
该阶段的完成标准是：DSH 可以在不更新或重启 DSH Desktop 的情况下更新，始终使用用户选择的 DSH Home，并在更新失败时恢复先前 Runtime。
