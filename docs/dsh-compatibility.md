...
| `unverified` | 尚未完成当前 Desktop 版本的完整验证，仅供 `system` 或 `custom` 来源的用户显式使用 |
...
| EMBED-002 | DSH Web UI 支持 WebView2、WebKitGTK 和 WKWebView                           | 跨平台桌面使用             | 特定平台交互或渲染异常 | 三个平台执行基本操作测试         |
| EMBED-003 | iframe 内的同源请求、Cookie、事件流和 WebSocket 不依赖普通浏览器顶层上下文 | 保持客户端与 DSH Host 通信 | iframe 可见但无法工作  | 创建会话并验证流式响应与设置保存 |

需要关注的响应策略包括 `Content-Security-Policy` 的 `frame-ancestors`、`X-Frame-Options`、Cookie 属性和浏览器信任校验。DSH Desktop 不向 iframe 注入 Tauri IPC 或本地文件权限。

## 数据与工作目录契约

| ID       | DSH 行为                                             | Desktop 用途                                    | 破坏后的影响               | 验证                                 |
| -------- | ---------------------------------------------------- | ----------------------------------------------- | -------------------------- | ------------------------------------ |
| DATA-001 | `DSH_HOME` 指定 DSH 配置、凭据、插件和会话数据根目录 | 隔离 Managed Process 数据并支持 generation 回滚 | 数据写入错误位置或回滚失效 | 使用临时 DSH Home 启动并检查写入位置 |
| DATA-002 | 启动进程的当前目录是默认文件系统位置                 | 为 Managed Process 提供确定的初始目录           | 工作区和工具默认位置异常   | 从指定目录启动并验证初始行为         |
| DATA-003 | 新版本能够读取或迁移目标 generation 中的数据         | DSH 热更新和失败回滚                            | 会话、配置或凭据不可用     | 复制 generation 后执行升级与回滚测试 |

数据格式兼容性与运行时代码兼容性分别验证。旧 Runtime 只与其原有 generation 配对，新 Runtime 在复制出的 generation 上启动。
...
8. 验证插件管理、重启、升级 generation 和回滚；
9. 更新兼容性清单与本文件引用的上游契约；
10. 保存测试使用的 DSH tag 或 commit，避免以浮动分支作为验证证据。

`system` 和 `custom` 来源可以报告 `unverified` 状态并由用户显式继续。`none` 来源和外部端点无法取得版本时，Host 只报告行为探测结果，不声明其版本兼容状态。
