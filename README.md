...
- `bundled` 单文件可执行程序内置 Node.js、DSH 和 pnpm，直接运行即可启动 DSH
- `bundled` 可在不更新桌面应用的情况下检查并安全更新已验证的内置 DSH 运行时
- `slim` 可执行程序体积更小，可使用系统中的 `dsh`、`npx`、自定义命令路径或已有服务
- 启动时按可配置的尝试顺序连接已有服务或启动新的 DSH
- 多次运行可执行文件时复用同一个 Host，并打开新的浏览器窗口
- 为当前窗口设置 DSH URL，也可以连接远程 DSH 服务
...
- `slim`：体积更小，适合已经安装 DSH、Node.js 或只连接已有服务的用户。
...
DSH Desktop 目前尚未发布稳定版本。希望试用当前版本时，可以按照[贡献指南](./CONTRIBUTING.md#构建可执行文件)从源码构建当前平台的 `bundled` 或 `slim` 可执行文件。
