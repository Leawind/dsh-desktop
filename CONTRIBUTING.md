...
应用默认连接 `http://127.0.0.1:3080`。启动前不需要手动运行 `dsh web`：应用会复用该地址上已有的 DSH 服务，或者通过全局设置选择系统中的 `dsh`、自定义 `dsh` 路径或 `npx` 启动服务。受管服务默认继承开发进程的 `DSH_HOME`，环境变量未设置时由 DSH 使用 `~/.dsh`；也可以在全局设置中指定自定义目录。
